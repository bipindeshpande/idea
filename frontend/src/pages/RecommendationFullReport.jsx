      ...
      {profileSummary.length > 0 && (
        <article className="rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-soft">
          <div className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
            <h2 className="text-lg font-semibold text-slate-900">Why these recommendations fit</h2>
            <p className="text-xs text-slate-500">
              How your inputs shaped the ranked shortlist
            </p>
          </div>
          <div className="mt-6 grid gap-6">
            {profileSummary.map((section, sectionIndex) => {
              // Determine theme based on section title
              const isDirection = section.title === "Direction";
              const isCapacity = section.title === "Capacity";
              const isStrengths = section.title === "Strengths";
              
              const sectionTheme = isDirection
                ? {
                ...
                : {
                    ...
                    icon: "text-aqua-600",
                    title: "text-aqua-700",
                    cardBorder: "border-aqua-300",
                    cardBg: "bg-white",
                    paramText: "text-aqua-800",
              ...
              const sectionIcon = isDirection ? "🎯" : isCapacity ? "⏳" : "💪";

              return (
                <div
                  key={sectionIndex}
                  className={`rounded-2xl border-2 ${sectionTheme.border} ${sectionTheme.bg} p-0 overflow-hidden shadow-md`}
                  style={{
                    borderColor: sectionTheme.borderColor,
                    backgroundColor: sectionTheme.bgColor,
                  }}
                >
                  <div
                    className={`flex items-center gap-2 px-5 py-3 ${sectionTheme.headerBg} border-b-2 ${sectionTheme.border}`}
                    style={{
                      backgroundColor: sectionTheme.headerBgColor,
                      borderBottomColor: sectionTheme.borderColor,
                    }}
                  >
                    <span className={`text-2xl ${sectionTheme.icon}`} style={{ color: sectionTheme.textColor }}>
                      {sectionIcon}
                    </span>
                    <h3
                      className={`text-sm font-bold uppercase tracking-wide ${sectionTheme.title}`}
                      style={{ color: sectionTheme.textColor }}
                    >
                      {section.title}
                    </h3>
                  </div>
                  <div className="p-5 grid gap-3 md:grid-cols-2">
                    {section.items.map(({ parameter, rationale }, itemIndex) => (
                      <div
                        key={itemIndex}
                        className={`rounded-lg border-2 ${sectionTheme.cardBorder} ${sectionTheme.cardBg} p-4 text-sm shadow-sm`}
                        style={{
                          borderColor: sectionTheme.borderColor,
                        }}
                      >
                        <p
                          className={`font-bold text-base ${sectionTheme.paramText}`}
                          style={{ color: sectionTheme.textColor }}
                        >
                          {parameter}
                        </p>
                        <p className="mt-2 text-slate-700 leading-relaxed">{rationale}</p>
                      </div>
                    ))}
                  </div>
                </div>
